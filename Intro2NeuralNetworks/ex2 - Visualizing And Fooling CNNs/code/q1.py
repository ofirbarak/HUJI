...
#
# Visualize a neuron from convolution layer.
# conv layer 2, index [0, 0, 0, 0]
#
def visualize_conv_neuron():
    conv2_neuron = conv2[0, 0, 0, 0]
    optimizer = tf.train.GradientDescentOptimizer(0.33)
    regularization = tf.norm(input_var)**2
    train = optimizer.minimize( -conv2_neuron + 1/100*regularization)
    sess = tf.Session()
    sess.run(init)
    max = 0
    image = None
    ...
    if image is None:
        ...
        img -= np.min(img)
        img /= np.max(img)
        scipy.misc.imsave("conv_visualization.jpg", img)
    

#
# Visualize FC neuron, from fc8 layer, index [0, 107]
# This pixel represents the probability of jellyfish, so we
# expect its visualization to be close to abstract jellyfish
#
def visualize_FC_neuron():
    fc7_neuron = fc8[0, 107]
    optimizer = tf.train.GradientDescentOptimizer(0.33)
    regularization = tf.norm(input_var)
    train = optimizer.minimize(-fc7_neuron*1000 + regularization)
    sess = tf.Session()
    sess.run(init)
    max = 0
    image = None
    ...
    if image is None:
        ...
        image -= np.min(image)
        image /= np.max(image)
        scipy.misc.imsave("fc_visualization.jpg", image)


visualize_conv_neuron()
