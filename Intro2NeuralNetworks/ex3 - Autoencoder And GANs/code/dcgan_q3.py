...
#g_params = [v for v in vars if v.name.startswith('decoder')]
...
G_solver = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.3).minimize(G_loss, var_list=g_params)

saver = tf.train.Saver(g_params)

with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    saver.restore(sess, './savedModel/decoder.ckpt')
    ...
    for e in range(EPOCHS):
        ...
        for i in range(iteration):
            ...
            rand = np.random.uniform(-1., 1., size=[BATCH_SIZE, 100])
